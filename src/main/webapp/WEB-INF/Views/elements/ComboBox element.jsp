            ...
            <c:forEach items="${og.options}" var="o">
                <c:forEach items="${options}" var="o2">
                    <c:if test="${o.id==o2.id}">
                        <li class="list-group-item text-white bg-success">
                            <b>${o2.name}</b><input class="right radio myClass" type="radio" price="${o2.priceMonthly}" optionName="${o2.name}"
                        <c:if test="${o2.existing==true}"> checked </c:if> priceOneTime="${o2.priceOneTime}" name="${og.name}" value="${o2.id}"><br>
                            <i>${o2.description}</i>
                            <p>Monthly price:<span class="right"><b>${o2.priceMonthly}$</b></span>
                             <c:if test="${o2.existing==false}"><br>One Time price:<span class="right"><b>${o2.priceOneTime}$</b></span> </c:if></p>

                            <div class="children" id="${o2.id}" style="display:none">
                                <c:forEach items="${children}" var="c">
                                    <c:if test="${o.id==c.parent.id}">
                                        <br>
                                        <h5 class="card-title">${c.name}<input class="right chk myClass1" right
                                                                              type="checkbox"
